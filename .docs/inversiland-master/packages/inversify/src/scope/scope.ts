...
export const tryGetFromScope: <T>(
  ...
  if (binding.scope === BindingScopeEnum.Singleton && binding.state.activated) {
    return binding.state.cache;
  }
...
const _saveToSingletonScope: <T>(
  ...
  binding.state.cache = result;
  binding.state.activated = true;
...
const _saveAsyncResultToSingletonScope: <T>(
  ...
  try {
    ...
    binding.state.cache = null;
    binding.state.activated = false;
