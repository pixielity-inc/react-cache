  ...
  switch (bindings.length) {
    case BindingCount.NoBindingsAvailable:
      if (target.isOptional()) {
        ...
        let msg: string = ERROR_MSGS.NOT_REGISTERED(container.id);
...
function _createSubRequests(
  ...
  activeBindings.forEach((binding: interfaces.Binding) => {
    ...
    if (
      ...
      const subContext: interfaces.Context = context.clone();

      subContext.container = binding.container || context.container;
