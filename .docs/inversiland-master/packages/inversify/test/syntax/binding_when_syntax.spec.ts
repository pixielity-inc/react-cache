...
describe("BindingWhenSyntax", () => {
  it("Should set its own properties correctly", () => {
    ...
    expect(_bindingWhenSyntax._binding.serviceIdentifier).toBe(ninjaIdentifier);
  });
  ...
  it("Should be able to configure custom constraint of a binding", () => {
    ...
    expect(binding.constraint(request)).toBe(true);
  });
  ...
  it("Should have false constraint binding null request whenTargetIsDefault", () => {
    ...
    expect(shurikenBinding.constraint(null)).toBe(false);
  });
  ...
  it("Should be able to constraint a binding to a named target", () => {
    ...
    expect(binding.constraint).not.toBeNull();
    ...
    expect(binding.constraint(request2)).toBe(false);
  });
  ...
  it("Should be able to constraint a binding to its parent", () => {
    ...
    expect(katanaBinding.constraint(katanaRequest)).toBe(true);
    expect(katanaBinding.constraint(shurikenRequest)).toBe(false);

    katanaBindingWhenSyntax.whenInjectedInto(Ninja);
    expect(katanaBinding.constraint(katanaRequest)).toBe(false);
    expect(katanaBinding.constraint(shurikenRequest)).toBe(true);

    shurikenBindingWhenSyntax.whenInjectedInto(Samurai);
    expect(shurikenBinding.constraint(katanaRequest)).toBe(true);
    expect(shurikenBinding.constraint(shurikenRequest)).toBe(false);

    shurikenBindingWhenSyntax.whenInjectedInto(Ninja);
    expect(shurikenBinding.constraint(katanaRequest)).toBe(false);
    expect(shurikenBinding.constraint(shurikenRequest)).toBe(true);
  ...
  describe("BindingWhenSyntax.when*Ancestor*()", () => {
    ...
    it("Should be able to apply a type constraint to some of its ancestors", () => {
      ...
      expect(shurikenBinding.constraint(woodShurikenRequest)).toBe(false);
      expect(shurikenBinding.constraint(ironShurikenRequest)).toBe(true);

      shurikenBindingWhenSyntax.whenAnyAncestorIs(NinjaStudent);
      expect(shurikenBinding.constraint(woodShurikenRequest)).toBe(true);
      expect(shurikenBinding.constraint(ironShurikenRequest)).toBe(false);

      katanaBindingWhenSyntax.whenAnyAncestorIs(SamuraiMaster);
      expect(katanaBinding.constraint(woodKatanaRequest)).toBe(false);
      expect(katanaBinding.constraint(ironKatanaRequest)).toBe(true);

      katanaBindingWhenSyntax.whenAnyAncestorIs(SamuraiStudent);
      expect(katanaBinding.constraint(woodKatanaRequest)).toBe(true);
      expect(katanaBinding.constraint(ironKatanaRequest)).toBe(false);
    });
