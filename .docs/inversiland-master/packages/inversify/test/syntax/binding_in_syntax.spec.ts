...
describe("BindingInSyntax", () => {
  it("Should set its own properties correctly", () => {
    ...
    expect(_bindingInSyntax._binding.serviceIdentifier).toBe(ninjaIdentifier);
  });
  ...
  it("Should be able to configure the scope of a binding", () => {
    ...
    expect(binding.scope).toBe(BindingScopeEnum.Transient);

    // singleton scope
    bindingInSyntax.inSingletonScope();
    expect(binding.scope).toBe(BindingScopeEnum.Singleton);
