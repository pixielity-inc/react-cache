...
describe("Lookup", () => {
  ...
  it("Should throw when invoking get, remove or hasKey with a null key", () => {
    ...
    expect(() => {
      lookup.get(invalid);
    }).toThrow(ERROR_MSGS.NULL_ARGUMENT);
    expect(() => {
  ...
  it("Should throws when key not found", () => {
    ...
    }).toThrow(ERROR_MSGS.KEY_NOT_FOUND);
    expect(() => {
  ...
  it("Should be clonable", () => {
    ...
    expect(copy.hasKey(key1)).toBe(true);
  ...
  it("Should use use the original non clonable entry if it is not clonable", () => {
    ...
    const copy: interfaces.Lookup<unknown> = lookup.clone();
    expect(copy.get(key1)[0] === warrior).toBe(true);
  });
  ...
  it("Should be able to remove a binding by a condition", () => {
    ...
    expect(lookup1.hasKey(warriorId)).toBe(true);
    expect(lookup1.hasKey(weaponId)).toBe(true);

    expect(lookup1.get(warriorId).length).toBe(2);

    expect(lookup1.get(weaponId).length).toBe(2);
    ...
    expect(lookup1.hasKey(warriorId)).toBe(true);
    expect(lookup1.hasKey(weaponId)).toBe(true);
    expect(lookup1.get(warriorId).length).toBe(1);
    expect(lookup1.get(weaponId).length).toBe(1);

    const lookup2: Lookup<Binding<unknown>> = getLookup();
    expect(lookup2.hasKey(warriorId)).toBe(true);
    expect(lookup2.hasKey(weaponId)).toBe(true);

    expect(lookup2.get(warriorId).length).toBe(2);

    expect(lookup2.get(weaponId).length).toBe(2);
    ...
    expect(lookup2.hasKey(warriorId)).toBe(false);
    expect(lookup2.hasKey(weaponId)).toBe(false);
  });
