...
describe("Provider", () => {
  it("Should support complex asynchronous initialization processes", (done) => {
          ...
          (
            ...
            void ninja.train().then((level: number) => {
              if (level >= 20) {
                ...
                reject(new Error("Not enough training"));
              }
            });
          }
        )
    ...
    void valueOrDefault(ninjaMasterProvider, {
      ...
      expect(ninjaMaster.rank).toBe("DefaultNinjaMaster");
    });

    void valueOrDefault(ninjaMasterProvider, {
      rank: "DefaultNinjaMaster",
    }).then((ninjaMaster: NinjaMaster) => {
      expect(ninjaMaster.rank).toBe("NinjaMaster");
      done();
    });
  });
  ...
  it("Should support the declaration of singletons", (done) => {
    ...
    void warriorProvider(10).then((warrior: Warrior) => {
      expect(warrior.level).toBe(10);

      void warriorProvider(10).then((warrior2: Warrior) => {
        expect(warrior2.level).toBe(20);
        done();
      });
    });
  });
