...
describe("Custom Metadata Reader", () => {
  ...
  it("Should be able to use extend the default metadata reader", () => {
    ...
    expect(Array.isArray(constructorMetadataLog)).toBe(true);

    expect(constructorMetadataLog.length).toBe(3);
    ...
    if (compilerGeneratedMetadata0) {
      expect(compilerGeneratedMetadata0[0]).toBe(Katana);
      expect(compilerGeneratedMetadata0[1]).toBe(Shuriken);
    }
    ...
    expect(propertyMetadataLog.length).toBe(3);

    expect(propertyMetadataLog[0] as interfaces.MetadataMap).toEqual({});
    expect(propertyMetadataLog[1] as interfaces.MetadataMap).toEqual({});
    expect(propertyMetadataLog[2] as interfaces.MetadataMap).toEqual({});
  });
});
